...
string function_for_reading_till_TAB(string data,int& index){
  string dheeraj_kumarBuffer = "";

  while(index<data.length() && data[index] != '\t'){
    dheeraj_kumarBuffer += data[index];
    index++;
  }
  index++;
  return dheeraj_kumarBuffer;
}
...
bool function_for_reading_the_intermediate_file(ifstream& readFile,bool& is_this_comment, int& lineNumber, int& address, int& blockNumber, string& label, string& opcode, string& operand, string& comment){
  ...
  if(is_this_comment){
    read_first_non_white_space(fileLine,index,dheeraj_kumarStatus,comment,true);
    return true;
  }
  address = convert_string_hex_to_int(function_for_reading_till_TAB(fileLine,index));
  dheeraj_kumarBuffer = function_for_reading_till_TAB(fileLine,index);
  if(dheeraj_kumarBuffer == " "){
    blockNumber = -1;
  }
  else{
    blockNumber = stoi(dheeraj_kumarBuffer);
  }
  label = function_for_reading_till_TAB(fileLine,index);
  opcode = function_for_reading_till_TAB(fileLine,index);
  if(opcode=="BYTE"){
    readByteOperand(fileLine,index,dheeraj_kumarStatus,operand);
  }
  else{
...
string for_creating_the_format_for_objectcode(){
  ...
  if(FORMAT_FLAG(operand)=='#'){//Immediate
    ...
    string dheeraj_kumarOperand = operand.substr(1,operand.length()-1);
    if(if_all_num(dheeraj_kumarOperand)||((SYMTAB[dheeraj_kumarOperand].exists=='y')&&SYMTAB[dheeraj_kumarOperand].relative==0)){//Imidiate integer value
      int IMMEDIATE_VALUE;

      if(if_all_num(dheeraj_kumarOperand)){
        IMMEDIATE_VALUE = stoi(dheeraj_kumarOperand);
      }
      else{
        IMMEDIATE_VALUE = convert_string_hex_to_int(SYMTAB[dheeraj_kumarOperand].address);
      }
    ...
    else if(SYMTAB[dheeraj_kumarOperand].exists=='n') {
      writing_the_data = "Line "+to_string(lineNumber);
      writing_the_data += " : Symbol doesn't exists. Found " + dheeraj_kumarOperand;
      write_to_file(errorFile,writing_the_data);
      object_code = convert_int_to_string_hexadecimal(convert_string_hex_to_int(OPTAB[REAL_OPCODE(opcode)].opcode)+1,2);
      object_code += (number_of_half_bytes==5)?"100000":"0000";
      return object_code;
      ...
      int operandAddress = convert_string_hex_to_int(SYMTAB[dheeraj_kumarOperand].address) + convert_string_hex_to_int(BLOCKS[BLocksNumToName[SYMTAB[dheeraj_kumarOperand].blockNumber]].startAddress);
      ...
      int relative_address_dheeraj = operandAddress - program_counter;


      if(relative_address_dheeraj>=(-2048) && relative_address_dheeraj<=2047){
        object_code = convert_int_to_string_hexadecimal(convert_string_hex_to_int(OPTAB[REAL_OPCODE(opcode)].opcode)+1,2);
        object_code += '2';
        object_code += convert_int_to_string_hexadecimal(relative_address_dheeraj,number_of_half_bytes);
        return object_code;
      }

      
      if(!nobase){
        relative_address_dheeraj = operandAddress - base_register_value;
        if(relative_address_dheeraj>=0 && relative_address_dheeraj<=4095){
          object_code = convert_int_to_string_hexadecimal(convert_string_hex_to_int(OPTAB[REAL_OPCODE(opcode)].opcode)+1,2);
          object_code += '4';
          object_code += convert_int_to_string_hexadecimal(relative_address_dheeraj,number_of_half_bytes);
          return object_code;
    ...
    string dheeraj_kumarOperand = operand.substr(1,operand.length()-1);
    if(dheeraj_kumarOperand.substr(dheeraj_kumarOperand.length()-2,2)==",X" || SYMTAB[dheeraj_kumarOperand].exists=='n'){//Error handling for Indirect with index based
      writing_the_data = "Line "+to_string(lineNumber);
      writing_the_data += (SYMTAB[dheeraj_kumarOperand].exists=='n')?": Symbol doesn't exists":" Index based addressing not supported with Indirect addressing";
      write_to_file(errorFile,writing_the_data);
    ...
    if(dheeraj_kumarOperand=="*"){
      dheeraj_kumarOperand = "X'" + convert_int_to_string_hexadecimal(address,6) + "'";

      the_record_that_needs_to_be_modified += "M^" + convert_int_to_string_hexadecimal(convert_string_hex_to_int(LITTAB[dheeraj_kumarOperand].address)+convert_string_hex_to_int(BLOCKS[BLocksNumToName[LITTAB[dheeraj_kumarOperand].blockNumber]].startAddress),6) + '^';
      the_record_that_needs_to_be_modified += convert_int_to_string_hexadecimal(6,2);
      the_record_that_needs_to_be_modified += '\n';
    }

    if(LITTAB[dheeraj_kumarOperand].exists=='n'){
      writing_the_data = "Line "+to_string(lineNumber)+" : Symbol doesn't exists. Found " + dheeraj_kumarOperand;
      write_to_file(errorFile,writing_the_data);
    ...
    int operandAddress = convert_string_hex_to_int(LITTAB[dheeraj_kumarOperand].address) + convert_string_hex_to_int(BLOCKS[BLocksNumToName[LITTAB[dheeraj_kumarOperand].blockNumber]].startAddress);
    program_counter = address + convert_string_hex_to_int(BLOCKS[BLocksNumToName[blockNumber]].startAddress);
    program_counter += (number_of_half_bytes==5)?4:3;
    ...
    int xbpe=0;
    string dheeraj_kumarOperand = operand;
    if(operand.substr(operand.length()-2,2)==",X"){
      dheeraj_kumarOperand = operand.substr(0,operand.length()-2);
      xbpe = 8;
    }

    if(SYMTAB[dheeraj_kumarOperand].exists=='n'){
      writing_the_data = "Line "+to_string(lineNumber)+" : Symbol doesn't exists. Found " + dheeraj_kumarOperand;
      write_to_file(errorFile,writing_the_data);
...
void pass2(){
  ...
  getline(intermediateFile, dheeraj_kumarBuffer); // Discard heading line
